...
with tab1:
    st.markdown('### 대시보드에 사용된 데이터 셋의 정보입니다')
...
with tab2:
    ...
    st.markdown('분포를 확인하고 싶은 연속형 혹은 범주형 특성을 선택해주세요')
    col1, col2, col3 = st.columns(3)
    with col1:
...
with tab3:
    st.markdown('### Correlation between Numeric')
    st.markdown('상관관계를 확인하고 싶은 연속형 변수를 선택해주세요')
    col1, col2, col3 = st.columns(3)
    with col1:
...
with tab4:
    st.markdown('### 미국 51개 주')
    st.markdown('미국 주별 특성을 확인할 수 있습니다')
    copy_data = data

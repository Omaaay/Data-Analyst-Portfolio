...
with tab1:
    ...
    with col1:
        ...
        st.slider("dti", min_value=0, max_value=100, key="dti")
    with col2:
...
with tab2:
    ...
    with col1:
        st.markdown('### Feature Select')
        st.slider("revol_util", min_value=0, max_value=100, key="revol_util")
        ...
        st.slider("mo_sin_rcnt_rev_tl_op", min_value=data.mo_sin_rcnt_rev_tl_op.min(), max_value=data.mo_sin_rcnt_rev_tl_op.max(), key="mo_sin_rcnt_rev_tl_op")
    with col2:
        st.markdown('### Feature Property')
        col1, col2 = st.columns([0.4,0.6])
        with col1:
...
with tab4:
    ...
    with col1:
        ...
        st.selectbox("sub_grade", help="대출등급", options=sorted(data.sub_grade.unique()), key="sub_grade")
    with col2:
        st.markdown('### Feature Property')
        col1, col2 = st.columns([0.4,0.6])
        with col1:
...
with tab5:
    ...
    with col11:
        col1, col2 = st.columns(2,gap='small')
        with col1:
            st.markdown("<center><h2>예측 결과</h2></center>", unsafe_allow_html=True)
            if restul_button:
    ...
    # st.divider()
    st.markdown('### 예측 결과 분석')
    st.markdown('대출 상환 여부를 예측하는데 어떤 요인들이 중요한지를 확인할 수 있어요')
    with st.expander(''):
        col1, col2 = st.columns([0.3,0.7],gap='small')
        if restul_button:
            with col1:
                privacy = predict_data.T.loc[['verification_status','addr_state','last_fico_range_high','last_fico_range_low','dti'],:]
        ...
        st.markdown('각 막대는 특정 요인이 상한 여부에 어떻게 영향을 미쳤는지 나타냅니다.')
        st.markdown('양수 막대는 미상환 확률을 높이는데 도움이 되는 요인이고, 음수 막대는 상환 확률을 줄이는데 영향을 주는 요인입니다.')
